...
import { startOrbitSiege } from "./orbit-siege.mjs?v=16";
...
const views = {
  ...
  galaxy() {
    ...
    return `<div class="map-wrap"><canvas id="starmap"></canvas>
      ...
      <button type="button" class="map-help-toggle" aria-expanded="false" aria-controls="map-travel-help" aria-label="Reisen zwischen Galaxien" title="Reisen zwischen Galaxien">i</button>
      <div class="map-status"><i></i><span id="map-view-title">ARCHIPEL</span><small>Alle Systeme sichtbar</small></div>
      ${seasonPanel}
      ...
      <aside id="map-travel-help" class="map-travel-help panel" hidden>
        <b>Reisen zwischen Galaxien</b>
        <p>Keine Luftlinie über die Karte. Die Route ist immer <strong>Anflug zum öffentlichen Sprungtor</strong>, dann der <strong>Sprung</strong>, dann der <strong>Zielflug</strong> in der anderen Galaxie.</p>
        <ul>
          <li>Warp-Forschung mindestens Stufe 1.</li>
          <li>Das Tor (Zentrum oder äußerer Hub) muss in deiner normalen Warp-Reichweite liegen.</li>
          <li>Der Sprung dauert 30 Minuten, mit Warp/Hyperspace höchstens auf 15 Minuten verkürzt, immer auf den Welt-Tick.</li>
          <li>Anflug und Zielflug rechnen wie ein Flug in derselben Galaxie. Treibstoff fällt für beide Beine plus den Sprung an.</li>
        </ul>
        <p>Tore sind auf der Karte als Hub markiert. Im Missionsfenster steht die genaue ETA, sobald Schiffe gewählt sind.</p>
      </aside>
...
async function bootMap() {
  ...
  state.map = createMap(canvas, selectSystem, ({ mode }) => {
    if (!stillHere()) return;
    const names = { sector: "ARCHIPEL", system: "GALAXIE", orbit: "SYSTEM" };
  ...
  const helpToggle=root.querySelector('.map-help-toggle'),help=root.querySelector('#map-travel-help');
  const closeSearch=()=>{mapTools?.classList.remove('open');searchToggle?.setAttribute('aria-expanded','false');if(searchToggle)searchToggle.textContent='⌕';};
  const closeFilters=()=>{if(filters)filters.hidden=true;filterToggle?.setAttribute('aria-expanded','false');};
  const closeHelp=()=>{if(help)help.hidden=true;helpToggle?.setAttribute('aria-expanded','false');};
  filterToggle?.addEventListener('click',()=>{closeSearch();closeHelp();filters.hidden=!filters.hidden;filterToggle.setAttribute('aria-expanded',String(!filters.hidden));});
  searchToggle?.addEventListener("click", () => {
    closeFilters();closeHelp();
  ...
  const paintSearchResults = () => {
    ...
    searchResults.innerHTML = matches.length
      ? matches.map((system) => {
          const galaxy = (state.galaxy?.regions || []).find((g) => g.id === Number(system.galaxyId || 0));
          return `<button type="button" data-search-system="${system.id}"><i aria-hidden="true"></i><span><b>${esc(system.name)}</b><small>${esc(galaxy?.name || "System")} · ${system.planetCount || system.planets || ""}</small></span><em>›</em></button>`;
        }).join("")
    ...
    travelTimer = setTimeout(async () => {
      try {
        ...
        box.innerHTML = `<b>Flugzeit ${t.ticks || ticksOf(t.ms, state.catalog)} Tick${(t.ticks || 1) === 1 ? "" : "s"}${extra ? " + Halt " + tickEta(extra, state.catalog) : ""}</b>
          ...
          <div>Distanz ${fmt(t.dist)} LE${t.sameSystem ? " · gleiches System" : t.hops ? ` · ${t.hops} Sprünge` : ""}${t.jumps ? ` · ${t.jumps} Galaxiesprung${t.jumps === 1 ? "" : "e"}` : ""}</div>
          ${t.intergalactic ? `<div class="hint">Route über öffentliches Sprungtor. Anflug und Zielflug rechnen wie bisher in der Galaxie; der Sprung dauert 15–30 Minuten (Warp/Hyperspace, nie unter 15 Min.) und wird auf den Welt-Tick gelegt.</div>` : ""}
