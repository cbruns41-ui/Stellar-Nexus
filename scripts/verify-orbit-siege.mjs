...
try{
 ...
 for(const [w,h] of [[390,844],[320,568],[844,390],[1280,800]]){
  await fresh(w,h);await start();await sleep(200);await shot(`overview-${w}-${h}`);await tap('.orbit-pause');const before=await view();
  await tap('[data-zoom="out"]');await tap('[data-zoom="out"]');const small=await view();assert.equal(small.W,before.W);assert.equal(small.planetR,before.planetR);assert.ok(small.zoom<before.zoom);assert.ok(small.cameraScale<before.cameraScale);assert.ok(before.planetR*before.cameraScale*2.4<45,"Planet stays small in overview");assert.ok(before.screenCx-before.arenaRadius*before.cameraScale>0&&before.screenCx+before.arenaRadius*before.cameraScale<w,"Entire attack area fits horizontally");
  ...
  assert.equal(Math.round(geometry.box.width),w,'Zoom never shrinks the window');assert.equal(Math.round(geometry.box.height),h);assert.ok(geometry.fits&&geometry.noOverlap,JSON.stringify({w,h,...geometry}));assert.ok(geometry.buttons.every(([bw,bh])=>bw>=43&&bh>=43));await tap('#pause-resume');await shot(`layout-${w}-${h}`);results.push({w,h,...geometry});
  ...
  if(w===390){
   await tap('[data-zoom="in"]');await tap('[data-zoom="in"]');const gestureBefore=await view();
   ...
   assert.ok(pinched.zoom<gestureBefore.zoom);assert.equal(pinched.W,gestureBefore.W);assert.equal(pinched.ringR,gestureBefore.ringR);assert.ok(await ev(`document.querySelector('#build-dock').hidden`),'Pinch must not select a slot');
   await send('Input.dispatchMouseEvent',{type:'mouseWheel',x:w/2,y:h*.65,deltaX:0,deltaY:-100});await sleep(100);assert.ok((await view()).zoom>pinched.zoom,'Wheel zooms into the battlefield');
   // Visible enemies at real world coordinates must scale with the camera; HUD and physics stay fixed.
   ...
   await ev(`(()=>{const g=document.querySelector('.orbit-game').orbitGame;g.mode='play';g.paused=true;g.wave=1;g.spawnLeft=0;g.waveLive=true;g.enemies=Array.from({length:8},(_,i)=>{const a=i*Math.PI/4;return{x:Math.cos(a)*600,y:Math.sin(a)*600,hp:100,max:100,r:30,heavy:false,flash:0,speed:0,wobble:0,trail:[],launchCd:0};});return true;})()`);await sleep(100);await shot('combat-overview');
   const original=await ev(`document.querySelector('.orbit-game').orbitGame.enemies.map(e=>[e.x,e.y,e.hp])`);const distant=await view();
   await tap('[data-zoom="in"]');await tap('[data-zoom="in"]');await tap('[data-zoom="in"]');const close=await view();await shot('combat-detail');
   assert.ok(close.cameraScale>distant.cameraScale*1.8);assert.deepEqual(await ev(`document.querySelector('.orbit-game').orbitGame.enemies.map(e=>[e.x,e.y,e.hp])`),original);
   await send('Emulation.setDeviceMetricsOverride',{width:844,height:390,deviceScaleFactor:1,mobile:true});await sleep(100);assert.deepEqual(await ev(`document.querySelector('.orbit-game').orbitGame.enemies.map(e=>[e.x,e.y,e.hp])`),original);assert.equal((await view()).planetR,close.planetR);
   await send('Emulation.setDeviceMetricsOverride',{width:w,height:h,deviceScaleFactor:1,mobile:true});await sleep(100);
   const range=await ev(`(()=>{for(let i=0;i<30;i++)document.querySelector('[data-zoom="in"]').click();const max=document.querySelector('.orbit-game').orbitView().cameraScale;for(let i=0;i<30;i++)document.querySelector('[data-zoom="out"]').click();const min=document.querySelector('.orbit-game').orbitView().cameraScale;return max/min;})()`);assert.ok(range>9,'Camera offers at least a ninefold zoom range');
   await send('Input.dispatchKeyEvent',{type:'keyDown',code:'Digit0',key:'0'});await send('Input.dispatchKeyEvent',{type:'keyUp',code:'Digit0',key:'0'});await ev(`document.querySelector('.orbit-game').orbitGame.paused=false;true`);
   const target=await view();const aim={id:1,x:target.screenCx-600*target.cameraScale,y:target.screenCy};await send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[aim]});await send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});
 ...
 await ev(`window.__slot=i=>{const root=document.querySelector('.orbit-game'),v=root.orbitView(),c=root.querySelector('canvas'),r=c.getBoundingClientRect(),a=-Math.PI/2+(i+.5)*Math.PI*2/v.slotN;const p={pointerId:7,clientX:r.left+(v.screenCx+Math.cos(a)*v.ringR*v.cameraScale)/v.W*r.width,clientY:r.top+(v.screenCy+Math.sin(a)*v.ringR*v.cameraScale)/v.H*r.height,bubbles:true};c.dispatchEvent(new PointerEvent('pointerdown',p));c.dispatchEvent(new PointerEvent('pointerup',p));};__slot(0);document.querySelector('[data-shop="laser"]').click();__advance(1);true`);
 ...
 const rangeCheck=await ev(`(()=>{const root=document.querySelector('.orbit-game'),g=root.orbitGame,v=root.orbitView(),a=-Math.PI/2+Math.PI/6,p={x:v.cx+Math.cos(a)*v.ringR,y:v.cy+Math.sin(a)*v.ringR};g.mode='play';g.spawnLeft=0;g.waveLive=false;const enemy=d=>({x:p.x+Math.cos(a)*d,y:p.y+Math.sin(a)*d,hp:100,max:100,r:10,speed:0,wobble:0,trail:[],flash:0});g.enemies=[enemy(240)];g.towers[0].cd=0;__advance(1);const inside=g.enemies[0].hp;g.enemies=[enemy(320)];g.towers[0].cd=0;__advance(1);return {inside,outside:g.enemies[0].hp};})()`);assert.ok(rangeCheck.inside<100,'Level 1 tower reaches 240 world units');assert.equal(rangeCheck.outside,100,'Range remains bounded');
