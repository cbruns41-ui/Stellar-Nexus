...
export async function verifyGameFlows({base,headers,databasePath,send,evaluate,until,pause}) {
  ...
  for(const filter of ['own','alliance','hostile','free'])assert.ok(await evaluate(`(()=>{const e=document.querySelector('[data-map-filter="${filter}"]').closest('label'),r=e.getBoundingClientRect();return r.top>=0 && r.right<=innerWidth && e.contains(document.elementFromPoint(r.x+r.width/2,r.y+r.height/2));})()`),'Visible first-tap filter '+filter);
  ...
  await until(`document.querySelector('#dead:not([hidden]) #again')`);
  await evaluate(`document.querySelector('#again').click()`);
  await until(`!document.querySelector('.orbit-game')`);
