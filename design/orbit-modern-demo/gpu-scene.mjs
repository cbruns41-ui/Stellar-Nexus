...
import {TURRET_ART,TURRET_WORLD_SCALE} from './turret-art.mjs';
import {createMissileVeil} from './missile-veil.mjs';
...
export function createGpuScene(root, getView) {
  ...
  function illustratedSprite(parent,file,w,h,pivotY=.5,keyed=false){
    const map=new T.TextureLoader().load('./assets/'+file);map.colorSpace=T.SRGBColorSpace;
  ...
  function illustratedFighter(){const g=new T.Group();illustratedSprite(g,'fighter-v3-key.png',136,136,.5,true);
  ...
  function illustratedTower(kind){const g=new T.Group(),gun=new T.Group();gun.name='gun';g.add(gun);
    ...
    return g;}
  const templates={fighter:illustratedFighter(),frigate:illustratedFrigate(),missile:missile(),interceptor:missile(true)};for(const k of ['laser','silo','flak','gauss','tesla','mine'])templates[k]=illustratedTower(k);
  function icon(kind){
  ...
  const earthTex=new T.TextureLoader().load('./assets/earth-blue-marble-july.jpg');earthTex.colorSpace=T.SRGBColorSpace;textures.add(earthTex);
  earthTex.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());
  const cloudTex=new T.TextureLoader().load('./assets/earth-clouds.jpg');cloudTex.wrapS=T.RepeatWrapping;textures.add(cloudTex);
  ...
  function render(game){if(disposed)return;const v=getView(),start=performance.now();
    ...
    planet.rotation.y=-1.55+game.time*.018;planetMat.uniforms.time.value=game.time;plumeMat.uniforms.time.value=game.time;shield.visible=game.hitFlash>.1;shield.scale.setScalar(1+(1-game.hitFlash)*.2);
    ...
    for(const t of game.towers){const m=object(t,t.kind);active.add(t);const a=-Math.PI/2+t.slot*Math.PI/3,x=Math.cos(a)*v.ringR,y=Math.sin(a)*v.ringR;m.position.set(x,-y,0);
    ...
    const player=object(playerKey,'gauss');active.add(playerKey);player.position.set(Math.cos(game.aim)*playerRadius,-Math.sin(game.aim)*playerRadius,20);player.scale.setScalar(playerScale);player.getObjectByName('gun').rotation.z=-game.aim-Math.PI/2;
    animateTurret(player,'gauss',game.flash);
